3/2 클로드 답변 <글 추가시 ai에게 전달하할 글>
글을 추가한다면은 너에게 뭐만 주면 되니? 예를 들어 사주 카테고리에 사주와 액땜이라는 글을 추가한다면은 내가 챗gpt나 ai에게 글 만들어온 후 너에게 어떤 형태로 주면 되니? 
  ⎿  ⧉ Selected 16 lines from md-docs\haha_sonnet46_flow.md in Antigravity

● 기존 글 구조를 보여드릴게요.

● Read 2 files (ctrl+o to expand)

● 구조를 파악했습니다. 필요한 게 꽤 있는데, 제가 최대한 단순하게 알려드릴게요.

  ---
  AI에게 글 만들 때 요청할 내용

  ChatGPT나 다른 AI에게 아래 형식으로 요청하세요:

  ---
  "사주와 액땜"에 대한 블로그 글을 다음 형식으로 작성해줘:

  1. 한국어 본문 (HTML 형식, <h2 id="...">, <h3>, <p>, <ul> 태그 사용, 3000자 이상)
  2. 영어 본문 (같은 내용 영어 번역, HTML 형식)
  3. 한국어 SEO 제목 (60자 이내)
  4. 영어 SEO 제목 (60자 이내)
  5. 한국어 메타 설명 (160자 이내)
  6. 영어 메타 설명 (160자 이내)
  7. 한국어 키워드 5개 (배열)
  8. 영어 키워드 5개 (배열)
  9. 목차(TOC) — 본문 h2 제목들의 id와 제목 (한/영 각각)
  10. FAQ 3개 (질문+답변, 한/영 각각)

  ---
  저에게는 이렇게 주시면 됩니다

  AI가 만든 글을 그냥 복붙해서 주시면 됩니다. 제가 코드 형식으로 변환해서 saju-posts.ts에 추가할게요.

  딱 하나만 기억하시면 됩니다:

  "한국어 본문 + 영어 본문 + SEO 정보"

  영어 번역이 없으면 한국어만 주셔도 됩니다. 제가 간단히 처리할 수 있습니다.

  ---
  본문 중간에 꼭 넣어달라고 할 것

  내부 링크용으로 AI에게 이렇게 요청하세요:

  "본문 중간 적절한 위치에 '사주팔자 무료 분석 버튼'을 넣어줘. 링크는 /ko"

  그러면 Gemini가 말한 내부 링크 SEO 효과도 자동으로 들어갑니다.
...
1. 내부 링크 ✅ 매우 중요, 바로 실행 가능

  Gemini 말이 100% 맞습니다. 현재 각 글의 contentKo 안에 HTML로 글을 쓰고 있어서 이미 이런 패턴이 있습니다:

  <a href="/ko">🔮 나의 타고난 사주팔자 무료로 분석하기 →</a>

  여기에 다른 블로그 글로 연결하는 링크를 추가하면 됩니다. 예:
  더 자세한 내용은 <a href="/saju/ohaeng">오행 완벽 가이드</a>를 확인해 보세요.

  비용 0원, 효과 큼 — 가장 우선순위 높은 작업입니다.

  ---
  2. 이미지 Alt 태그 ⚠️ 맞지만 현실적 제약 있음

  현재 블로그 글은 이미지가 없는 순수 텍스트 HTML입니다. 이미지를 추가하려면:
  - 이미지 파일을 public/ 폴더에 업로드
  - contentKo에 <img> 태그 삽입

  지금 당장보다는 글 수가 안정된 후에 할 작업입니다. 텍스트 콘텐츠 품질이 먼저입니다.

  ---
  3. publishedAt 날짜 업데이트 ⚠️ 반은 맞고 반은 틀림

  주의할 점이 있습니다.

  - 글을 실질적으로 크게 수정했을 때 → 날짜 업데이트 ✅ 맞음
  - 내용 변경 없이 날짜만 바꾸는 것 → Google이 감지하면 오히려 신뢰도 하락 ❌

  현재 모든 글이 2026-02-22 날짜인데, 실제로 큰 업데이트를 했을 때만 바꾸세요.
  ...
  │ 1위  │ 내부 링크 추가       │ 쉬움   │ 높음             │
  ├──────┼──────────────────────┼────────┼──────────────────┤
  │ 2위  │ 글 추가 (카테고리별) │ 중간   │ 매우 높음        │
  ├──────┼──────────────────────┼────────┼──────────────────┤
  │ 3위  │ 이미지 + Alt 태그    │ 어려움 │ 중간             │
  ├──────┼──────────────────────┼────────┼──────────────────┤
  │ 4위  │ 날짜 업데이트        │ 쉬움   │ 낮음 (남용 금지) │
  └──────┴──────────────────────┴────────┴──────────────────┘

  지금 당장 가장 효과적인 건 기존 글에 내부 링크 추가 + 새 글 작성입니다. 혹시 내부 링크 추가 작업 진행할까요?

❯ publishedAt 날짜 업데이트는 내가 날짜를 쳐 넣어야 해? 아니면 글을 수정하면 구글이 알아서 찾는거야? 
...
● 두 가지를 구분해야 합니다.
  ...
  직접 수정해야 합니다. lib/blog/saju-posts.ts 같은 파일 안에 이렇게 적혀있는 것:

  publishedAt: '2026-02-22',

  이걸 직접 '2026-03-02'로 바꿔야 합니다. 자동으로 바뀌지 않습니다.
  ----------------------------
