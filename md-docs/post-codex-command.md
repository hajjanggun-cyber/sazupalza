...
조건부 추가 확인:
- 시리즈 글이면 `## 14. Series Cannibalization Prevention Rules`와 `## 15. Series H2 Template Rules`를 반드시 본다.
...
17. 시리즈 글의 H2가 `## 15. Series H2 Template Rules` 문장을 그대로 복사하지 않고, 정보 단위만 가져와 고유 제목으로 바뀌었는지
...
이 섹션의 H2는 복사 가능한 제목 목록이 아니라, 시리즈 글에서 빠뜨리지 말아야 할 정보 단위다. 실제 `contentKo`, `contentEn`, `toc`, `tocEn`에 들어가는 H2 제목은 매 글의 검색 의도와 고유 차별점에 맞게 다시 쓴다.

[DO]
- 시리즈 템플릿은 "무엇을 다룰지"를 정하는 기준으로만 사용한다.
- H2 제목, H2 순서, 예시, CTA 위치, FAQ 질문은 같은 시리즈 안에서도 글마다 바꾼다.
- 새 시리즈 글을 쓰기 전 같은 시리즈의 직전 3개 글 H2를 확인하고, 제목 문장과 전개 순서가 과하게 같으면 바꾼다.
- 일주/일간 글은 각 글의 핵심 차별 포인트를 H2에 반영한다. 예: 외로움, 과열, 충돌, 직업 압박, 관계 방어, 감정 소모 등.

[NOT]
- 아래 코드블록의 H2 문장을 그대로 복사하고 일간명·일주명만 바꾸는 방식 금지.
- 모든 일주 글을 `기본 에너지 -> 성격 -> 직업·재물 -> 연애 -> 2026` 순서로만 고정 금지. 검색 의도상 필요한 경우 2026, 관계, 직업, 한계 설명을 앞쪽에 배치할 수 있다.
